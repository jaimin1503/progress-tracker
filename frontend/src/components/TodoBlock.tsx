...
const TodoBlock = () => {
  ...
  const handleInputChange = (todo_idx:number, index:number, updatedValue:string) => {
    const updatedTodos = [...todos];
    updatedTodos[todo_idx].tasks[index].content = updatedValue;
    setTodos(updatedTodos);
  };

  const handleTitleChange = (todo_idx:number, updatedValue:string) => {
    const updatedTodos = [...todos];
  ...
  const handleCheckboxChange = (checked:boolean, todo_idx:number, index:number, todo_id:string, task_id:string) => {
    const updatedTodos = [...todos];
  ...
  const handleKeyDown = (event, todo_id:string, updatedValue:string) => {
    if (event.key === "Enter") {
      axios
        .put(
  ...
  const handleKeyPress = (event, todo_id:string, task_id:string, updatedValue:string) => {
    if (event.key === "Enter") {
      axios
        .put(
          `http://localhost:5555/user/todos/${todo_id}/tasks/${task_id}`,
